...
- Size note: this guide is `small-guide` (about `17 KiB`, `173` rendered lines); known generator inputs are `project-map.json` `11 KiB` and `evidence-index.jsonl` `1 KiB`.
...
- Evidence records: `2` records in `evidence-index.jsonl`; this overview is presentation only.
