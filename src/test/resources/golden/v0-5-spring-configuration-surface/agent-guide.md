...
- Size note: this guide is `small-guide` (about `26 KiB`, `244` rendered lines); known generator inputs are `project-map.json` `20 KiB` and `evidence-index.jsonl` `4 KiB`.
...
- Source-backed fact surfaces: endpoints `0`, direct Spring components `1`, Spring application surface rows `6`, entities `0`, embeddables `0`, tests `0`.
- Planning/navigation surfaces: warnings `0`, quality/change-risk hints `6`, local documents `0`, document reconciliation hints `0`.
...
- No represented rows for: `Spring MVC endpoints`, `domain/data model facts`, `test classes`, `local project documentation`, `generated-source root metadata`. This means the current deterministic scan emitted no rows for those surfaces; it does not prove the runtime behavior is absent outside the supported analyzer scope.
