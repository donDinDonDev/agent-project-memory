...
Generated deterministically from `project-map.json` and `evidence-index.jsonl` for the `codex` profile. The profile generator does not re-analyze source files.

## Profile Operating Notes

- Intended use: Local coding-agent implementation, review, and verification planning.
- First-pass reading order: `artifact-set.json`, `agent-guide.md`, `project-map.json`, `evidence-index.jsonl`, `endpoints.md`
- Change posture: Inspect cited evidence before edits; keep changes tied to existing facts and explicit uncertainty.
- Evidence posture: Prefer file/line evidence references over narrative claims; do not treat this Markdown as new evidence.
- Selector: `codex`; artifact path: `agent-profiles/codex.md`
...
- `agent-guide.md`: generic deterministic guide generated from the same source artifacts; this profile narrows the presentation for `codex`.
...
- Start from the affected section in `agent-guide.md`, then verify the same fact IDs in `project-map.json` and `evidence-index.jsonl`.
- Before changing code, open the first cited source path for each affected fact category.
- When planning tests, keep `no_obvious_test` and `uncertain_planning_hint` as planning hints, not coverage proof.
