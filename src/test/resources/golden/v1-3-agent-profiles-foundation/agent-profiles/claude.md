...
Generated deterministically from `project-map.json` and `evidence-index.jsonl` for the `claude` profile. The profile generator does not re-analyze source files.
...
- `agent-guide.md`: generic deterministic guide generated from the same source artifacts; this profile narrows the presentation for `claude`.
...
- Summarize by section only after checking the corresponding evidence pointer in this profile.
- Keep document-backed hints separate from code-backed facts in any handoff or review.
- Call out not-analyzed areas explicitly instead of filling them from general framework knowledge.
